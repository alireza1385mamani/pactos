...
IDEN_REQ_HID ="\x00"*(BUFFER_SIZE_HID-1) + "\x82"
...
KEY_VALUES_BJ2 =     [3, 6,  8, 16, 2, 5, 17, 15, 14, 9, 12, 10, 4, 7, 13, 11]
KEY_VALUES_BJ3 =     [3, 6, 11, 10, 2, 5,  9,  8,  0, 0,  0,  0, 4, 7, 13, 12]
KEY_VALUES_BJNT =    [3, 6, 19, 16, 2, 5, 17, 15, 14, 9, 18, 10, 4, 7, 20, 11, 12, 21, 13, 22]
KEY_VALUES_BJNT_78 = [3, 6, 19, 16, 2, 5, 17, 15, 14, 9, 24, 10, 4, 7, 23, 11, 12, 21, 13, 22]
...
####
#device_model = 'bj_hooshmand'

class OptionsPanel(gui.SettingsPanel):
	title = _("Pactos Settings")

	def makeSettings(self, settingsSizer):
		sHelper = gui.guiHelper.BoxSizerHelper(self, sizer=settingsSizer)
		
		iniFilePath = os.path.join(PLUGIN_DIR,"bjSettings","configs.ini")
		self.pactos_settings = ConfigObj(iniFilePath, indent_type="\t", encoding="UTF-8", file_error=False)
		# Python converts \r\n to \n when reading files in Windows, so ConfigObj can't determine the true line ending.
		self.pactos_settings.newlines = "\r\n"
		
		self.device_list = ['bj_hooshmand', 'bj4', 'bj2', 'bj2_8', 'bj2_11', 'bj3', 'bj3_8', 'bj3_11']
		self.deviceComboBox = sHelper.addItem(
			wx.ComboBox(self, choices=self.device_list)
		)
		self.default_device = self.pactos_settings['default_device']
		self.deviceComboBox.SetSelection(self.device_list.index(self.default_device))
		paGlobals.device_model = self.default_device

	def onSave(self):
		paGlobals.device_model = self.device_list[self.deviceComboBox.GetSelection()]
		self.pactos_settings['default_device'] = paGlobals.device_model
		self.pactos_settings.write()
...
class BrailleDisplayDriver(braille.BrailleDisplayDriver,ScriptableObject):
	...
	def sync_keyboards(self):
		...
		if lid == 69796905 or lid == -255851479:
			...
			config.conf["braille"]["inputTable"]="pactos-fa-ir-g1.utb"
			config.conf["braille"]["translationTable"]="pactos-fa-ir-g1.utb"
...
#			braille.handler.update()
	...
	def __init__(self):
		...
		braille.filter_displaySize.register(self.get_display_size)
		
		brailleTables.addTable(os.path.join(PLUGIN_DIR,"bjSettings","pactos-fa-ir-g1.utb"), _("pactos persian grade 1"))
		brailleTables.addTable(os.path.join(PLUGIN_DIR,"bjSettings","pactos-en-us-g1.ctb"), _("pactos english grade 1"))
		
		#self.numCells = 20
		self.bt_read_thread = threading.Thread(target=self.bt_read)
		self.reorder = False
		self.isMod78 = False
		self.healthy_connection = False
		self.model = paGlobals.device_model
		...
		try:
			paGlobals.search_bluetooth = self.pactos_settings['bt_name'] != 0
		except KeyError as e:
			paGlobals.search_bluetooth = False
			
		try:
			paGlobals.lang1 = self.pactos_settings['lang1']
		except KeyError as e:
			paGlobals.lang1 = 'pactos-fa-ir-g1.ctb'
			
		try:
			paGlobals.lang2 = self.pactos_settings['lang2']
		except KeyError as e:
			paGlobals.lang2 = 'pactos-en-us-g1.ctb'
			
		try:
			paGlobals.device_model = self.pactos_settings['default_device']
			self.model = paGlobals.device_model
		except KeyError as e:
			pass
...
#		self.sync_keyboards()
		...
		for portType, dev_adr in _getPorts():
			...
			if self.healthy_connection:
				# A display responded.
				self.numCells = int(self.pactos_settings[self.model]['num_cells'])
				self.reorder = int(self.pactos_settings[self.model]['reorder_bits'])==1
				self.key_map = [int(x) for x in list(self.pactos_settings[self.model]['key_map'])]
				if self.reorder:
					try:
						self.reorder_sequence = [int(x) for x in self.pactos_settings[self.model]['reorder_sequence']]
					except KeyError as e:
						pass
					
				log.info("LLLLLLLLLLLLLLLLLLLLLLLLLL    {map} LLLLLLLLLLLLLLLLLLLLLLL".format(map=self.key_map))
		...
		paGlobals.commands_file = os.path.join(PLUGIN_DIR,"bjSettings","commands_" + self.model + ".ini")
		paGlobals.gestures_file = os.path.join(PLUGIN_DIR,"bjSettings","gestures_" + self.model + ".ini")
	...
	def change_device_model(self, new_model):
		log.info("CCCCCCCHHHHHHHAAAAAANNNNNNGGGGGGIIIIINNNNGGGGGG   TTTOOOOOOO {mode}".format(mode=new_model))
		...
		if self.reorder:
			try:
				...
				pass
		log.info("numCells = {numCells}, reorder = {reorder}".format(numCells=self.numCells, reorder=self.reorder))
		log.info("CCCCCCCHHHHHHHAAAAAANNNNNNGGGGGGIIIIINNNNGGGGGG   TTTOOOOOOO {mode}".format(mode=new_model))
...
#		self.sync_keyboards()
		...
		if self.model != paGlobals.device_model :
			self.change_device_model(paGlobals.device_model)
		
		num_bytes = 3 if self.isHid else 4
		...
		if self.model.startswith("bj2"):
			d7 = 0x0040
			d8 = 0x0004
		elif self.model.startswith("bj3"):
			d7 = 0x0080
			d8 = 0x0040
		else:
			d7 = 0x4000
		...
		tmpSet = set()
		if route_key != 0xff:
			if (self.model in ["bj4", "bj_hooshmand"]) and route_key==21:
				tmpSet.add(12)
			elif (self.model in ["bj4", "bj_hooshmand"]) and route_key==22:
				tmpSet.add(21)
			elif (self.model in ["bj4", "bj_hooshmand"]) and route_key==23:
				tmpSet.add(13)
			elif (self.model in ["bj4", "bj_hooshmand"]) and route_key==24:
				tmpSet.add(22)
			else:
		...
		if not self.isMod78  and keys == (d7 | d3):
			...
			ui.message(_("ورود به حالت ۸ نقطه"), speechPriority=2)
			return
		if self.isMod78  and keys == (Control | d6):
			self.isMod78 = False
			#speech.speakMessage(u"خروج از حالت ۸ نقطه")
			ui.message(_("خروج از حالت ۸ نقطه"), speechPriority=2)
			return
		...
		if keys == (Control|Space):
			if config.conf["braille"]["inputTable"]== "pactos-fa-ir-g1.utb":
		...
		while idx > 0:
			for i in range(8):
				...
				if key_pressed:
					tmpSet.add(self.key_map[offset+i])
			offset += 8
			idx -= 1
	...
	def reorder_cells(self,true_cells):
		...
		for cell in true_cells:
			for i in range(0, len(self.reorder_sequence),2):
				first = self.reorder_sequence[i]
				second = self.reorder_sequence[i+1]
				cell = self.swap_bits(cell,first,second,1)
			reordered_cells.append(cell)
				
		return reordered_cells #"".join(chr(c) for c in reordered_cells)
		
	def get_display_size(self, cell_size):
	...
	def display(self, cells:List[int]):
		...
		if self.isHid :
			...
			data = bytes([0]+cells) + b"".join([intToByte(x) for x in APPEND])
